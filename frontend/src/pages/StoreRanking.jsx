...
/* ─── 单个 App 卡片行（Insightrackr 风格：大图标 + 小文字） ─── */
function AppCard({ app, index, onClick }) {
  ...
  return (
    ...
    >
      {/* 排名号 */}
      <span className={`app-rank ${isTop3 ? "rank-top3" : ""}`}>
        {rank}
      </span>
...
export default function StoreRanking() {
  ...
  const [dataPaid, setDataPaid]   = useState([]);   // 付费榜数据
    ...
    try {
      ...
      if (store === "app_store") {
        ...
        const [asRes, gpRes] = await Promise.all([
          axios.get(`${API}/api/appstore/top20`, { params: { category } }),
  ...
  const currentCatLabel = CATEGORIES.find(c => c.key === category)?.zh || category;
  ...
  return (
    <div className="page-container">
      ...
      {/* ─── 筛选工具栏（Insightrackr 风格） ─── */}
      <div className="ranking-toolbar">
        {/* 日期标签 */}
        <div className="toolbar-date">
          <span className="toolbar-icon"><BarChartOutlined /></span>
        ...
        >
          ...
          <option value="combined">合并 / Combined</option>
        </select>
        ...
        <select
          ...
          className="toolbar-select toolbar-country"
        >
          {COUNTRIES.map(c => (
            <option key={c.key} value={c.key}>{currentCountryFlag} {c.en}</option>
          ))}
        </select>
      ...
      <div className="ranking-hint">
        提示：点击应用查看详情及广告创意思路 / Tap an app to view details & ad creative ideas
      </div>
      ...
      <div className="ranking-grid">
        ...
        <div className="ranking-column">
          <div className="column-header">
            <span className="column-icon free">🆓</span>
        ...
        <div className="ranking-column">
          <div className="column-header">
            <span className="column-icon paid">💰</span>
      ...
      {!loading && data.length > 0 && (
        <div className="ranking-footer">
          <span>共 {data.length} 款应用 · {currentCatLabel} · {COUNTRIES.find(c=>c.key===country)?.zh||country}</span>
